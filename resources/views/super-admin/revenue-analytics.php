...
<div class="page fu">
  ...
  <div class="stat-grid">
    <div class="card">
      ...
      <div class="stat-val" id="mrrVal">...</div>
    ...
    <div class="card">
      ...
      <div class="stat-val" id="arrVal">...</div>
      <div class="stat-lbl">Annual Recurring Revenue (ARR)</div>
      <div class="stat-sub" id="activeSubCount"><i class="fa fa-circle" style="color:#3b82f6;font-size:7px"></i> ... active subscriptions</div>
    </div>
    <div class="card">
      ...
      <div class="stat-sub"><i class="fa fa-minus" style="color:var(--text-light)"></i> 7–8 tenants at risk</div>
    </div>
    <div class="card">
...
(function() {
    ...
    if (movRows) {
        movRows.innerHTML = [
            {l: "New signups", v: "+12", c: "#16a34a"}, {l: "Upgrades", v: "+5", c: "#3b82f6"},
            {l: "Downgrades", v: "-2", c: "#d97706"}, {l: "Churned", v: "-7", c: "#e11d48"},
    ...
    if (signupList) {
        signupList.innerHTML = signups.map((s, idx) => `<div class="tl-item" style="display:flex;align-items:center;gap:12px;padding:12px 0;${idx === signups.length - 1 ? '' : 'border-bottom:1px solid #f1f5f9;'}"><div class="tl-dot ic-green" style="width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:13px"><i class="fa fa-user-plus" style="font-size:13px"></i></div><div style="flex:1"><div style="font-size:13.5px;font-weight:700">${s.n}</div><div style="font-size:11px;color:var(--text-light)">${s.prov} · ${s.d}</div></div><span class="plan-badge ${planBadge[s.p]}" style="font-size:10px">${planIcon[s.p]} ${s.p}</span></div>`).join("");
    }
    ...
    if (s) {
        if (document.getElementById('mrrVal')) document.getElementById('mrrVal').textContent = 'NPR ' + (s.mrr / 100000).toFixed(2) + 'L';
        if (document.getElementById('arrVal')) document.getElementById('arrVal').textContent = 'NPR ' + (s.mrr * 12 / 100000).toFixed(2) + 'L';
        if (document.getElementById('activeSubCount')) document.getElementById('activeSubCount').innerHTML = `<i class="fa fa-circle" style="color:#3b82f6;font-size:7px"></i> ${planTotal} active subscriptions`;
    }
})();
</script>
